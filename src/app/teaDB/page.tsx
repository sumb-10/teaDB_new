...
import Radar5 from "./Radar5Client";
...
type Assessment = {
  ...
  // 향미(0–5, 0.5)
  aroma_length?: number; delicacy?: number; continuity?: number; aftertaste?: number; refinement?: number;
  created_at?: string; updated_at?: string;
};
const BODY_METRICS: Array<keyof Assessment> = ["thickness", "density", "smoothness", "clarity", "granularity"];
const AROMA_METRICS: Array<keyof Assessment> = ["aroma_length", "delicacy", "continuity", "aftertaste", "refinement"];
...
function toArrayOfObject<T>(data: unknown): T[] {
  if (Array.isArray(data)) {
    return data.filter((x): x is T => x !== null && typeof x === "object" && !Array.isArray(x));
  }
...
async function findTeaById(teaId: string): Promise<Tea | null> {
  ...
  for (const file of files) {
    try {
      const parsed = await readJson(path.join(teaDir, file));
      const arr = toArrayOfObject<Tea>(parsed);
...
async function collectAssessmentsByTeaId(teaId: string): Promise<Assessment[]> {
  ...
  for (const file of files) {
    try {
      const parsed = await readJson(path.join(asmDir, file));
      const arr = toArrayOfObject<Assessment>(parsed);
      for (const a of arr) if (a.tea_id === teaId) result.push(a);
    } catch {}
  }
  return result;
}
...
/** ---------- Page ---------- */
// ❌ (기존) searchParams: Promise<...>  →  ✅ (수정) 동기 객체로 받기
export default async function TeaDBPage({
  ...
  const teaId = typeof idParam === "string" ? idParam : undefined;
  if (!teaId) notFound();
  ...
  for (const key of BODY_METRICS) {
    const values = assessments.map((a) => a[key]).filter((v): v is number => typeof v === "number");
  ...
  return (
    <main className="p-6 max-w-3xl mx-auto">
      ...
      <p className="text-sm text-gray-500 mb-6">
        ...
        {typeof tea.production_year === "number" && <> · 생산연도: {tea.production_year}</>}
